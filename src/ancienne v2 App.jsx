...
async function fetchSpeciesDetail(speciesUrl) {
  try {
    ...
    const d = await res.json();
    const desc = d.flavor_text_entries.find(e => e.language.name === "fr")?.flavor_text
              || d.flavor_text_entries.find(e => e.language.name === "en")?.flavor_text
              || "";
...
async function fetchEvolutionChain(url) {
  try {
    ...
    const walk = async (node) => {
      ...
      let trigger = null;
      if (detail) {
        if (detail.min_level) trigger = `Niv. ${detail.min_level}`;
        else if (detail.item) trigger = detail.item.name.replace(/-/g," ");
        else if (detail.trigger?.name === "trade") trigger = "Échange";
        else if (detail.trigger?.name === "use-item" && detail.item) trigger = detail.item.name.replace(/-/g," ");
        else if (detail.min_happiness) trigger = `Bonheur ${detail.min_happiness}`;
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding:"0 24px 32px", display:"flex", flexDirection:"column", gap:24 }}>
          ...
          {/* Stats */}
          <div>
            <div style={{ fontSize:12, fontWeight:800, color:"#A89FC0", letterSpacing:"0.08em", marginBottom:10 }}>STATISTIQUES</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {poke.stats.map(s => <StatBar key={s.name} name={s.name} value={s.value} />)}
              <div style={{ display:"flex", alignItems:"center", gap:10, borderTop:"1px solid #F0EDF8", paddingTop:8, marginTop:4 }}>
                <div style={{ width:80, fontSize:11, color:"#A89FC0", fontWeight:600, textAlign:"right" }}>Total</div>
                <div style={{ width:30, fontSize:13, fontWeight:800, color:"#6C4FDF", textAlign:"right" }}>
                  {poke.stats.reduce((a,s)=>a+s.value,0)}
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Abilities */}
          <div>
            <div style={{ fontSize:12, fontWeight:800, color:"#A89FC0", letterSpacing:"0.08em", marginBottom:10 }}>TALENTS</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {poke.abilities.map(a => (
                <div key={a.name} style={{
                  padding:"6px 14px", borderRadius:20,
                  background: a.hidden ? "#F5F3FF" : "#F0EDF8",
                  border: a.hidden ? "1.5px dashed #6C4FDF" : "1.5px solid #E8E4F0",
                  fontSize:12, fontWeight:600, color: a.hidden ? "#6C4FDF" : "#4A3F6B",
                  textTransform:"capitalize",
                }}>
                  {a.name} {a.hidden && <span style={{ fontSize:10, opacity:0.7 }}>(caché)</span>}
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}
      ...
      {loading ? (
        ...
        <div style={{ display:"flex",gap:32,alignItems:"center" }}>
          {[["left",left],["right",right]].map(([side,poke],si) => {
            ...
            return (
              <div key={side} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:8 }}>
                <div style={{ fontSize:11,color:"#A89FC0",fontWeight:600,letterSpacing:"0.06em" }}>
                  {side==="left"?"← GAUCHE":"DROITE →"}
                </div>
                <div onClick={()=>vote(side)}
                  style={{ width:240,background:"#fff",borderRadius:24,
                    border:`2.5px solid #E8E4F0`,padding:"20px 16px 24px",cursor:"pointer",
                    transition:"border-color 0.2s,box-shadow 0.2s",
                    boxShadow:"0 4px 20px rgba(0,0,0,0.08)",
                    display:"flex",flexDirection:"column",alignItems:"center",gap:12,
                    userSelect:"none", ...swipeStyle }}
                  ...
                  <div style={{ width:150,height:150,borderRadius:20,background:tc.light,
                ...
                <div style={{ padding:"6px 16px",borderRadius:12,background:"#F5F3FF",color:"#6C4FDF",fontSize:12,fontWeight:600 }}>
                  ELO {data.elos[poke?.id]||INITIAL_ELO}
                </div>
              </div>
            );
          })}
